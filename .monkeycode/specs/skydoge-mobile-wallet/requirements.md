...
### Requirement 5: 0.1% 手续费捐赠功能

**User Story:** AS a user, I want to automatically donate 0.1% of each transaction as a fee to a specified blockchain address, so that I can support the Skydoge project development.

#### Acceptance Criteria

1. WHEN user initiates a send transaction, the system SHALL automatically calculate 0.1% of the transaction amount as a donation fee.
2. THE donation fee SHALL be sent to the specified fee address: `SfeeAddressForDonation1234567890abcd` (链上硬编码地址).
3. WHEN user sends amount X, the system SHALL send X - 0.1%*X to the recipient, and 0.1%*X to the fee address.
4. THE system SHALL display the donation fee amount before transaction confirmation.
5. THE system SHALL allow user to enable/disable the donation feature in settings.
6. IF user has disabled the donation feature, the system SHALL NOT include the 0.1% fee in transactions.
...
**Bug Fix:**
- Fixed WalletBloc initialization bug: WalletWrapper now dispatches `CheckWalletExistsEvent` in `initState()` to properly transition from `WalletInitial` state.

**New Feature:**
- **Multi-language Support**: Added English and Simplified Chinese (简体中文) language support. Users can switch languages in the Settings screen. All UI text is now localized.

#### New Acceptance Criteria for Multi-language

1. THE system SHALL support English (en) and Simplified Chinese (zh) languages.
2. WHEN user changes language in settings, the system SHALL persist the preference.
3. THE system SHALL display all UI text in the selected language.
4. THE system SHALL use the device locale as the default language if supported.
